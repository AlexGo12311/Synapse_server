...
let token = null, userId = null, ws = null;
let activeTargetId = null, activeTargetName = null, currentTab = 'chats';
...
const WS_MAX_RECONNECT = 10, WS_RECONNECT_BASE_DELAY = 1000, WS_RECONNECT_MAX_DELAY = 30000;
let chatSearchMatches = [], chatSearchActiveIndex = -1;
let allUsersList = [];
let typingTimer = null, isTyping = false, peerTypingTimer = null;
...
function setLanguage(lang) {
    ...
    document.querySelectorAll('.lang-btn').forEach(btn => btn.classList.toggle('active', btn.textContent.trim().toLowerCase() === lang));
    document.querySelectorAll('.lang-option').forEach(btn => btn.classList.toggle('active', btn.dataset.lang === lang));
    const statusBar = document.getElementById('statusBar');
    if (statusBar) updateConnectionStatus(statusBar.className.match(/status-(\w+)/)?.[1] || 'connecting');
    renderChatsList();
...
let currentThemeIndex = 0, currentThemeMode = 'light';
const AVATAR_COLORS = ["#FF6B6B", "#4ECDC4", "#45B7D1", "#FFA07A", "#98D8C8", "#F7B731", "#A8E6CF", "#FF8B94", "#B39DDB", "#81C784", "#FFD54F", "#4DB6AC", "#FF8A65", "#BA68C8", "#64B5F6", "#AED581", "#FFB74D", "#F06292", "#7986CB", "#4DD0E1"];
...
async function updateMyFingerprintDisplay() {
    ...
    if (!el) return;
    if (!publicKeyBase64) { el.textContent = 'Loading...'; return; }
...
function openChatSearch() {
    ...
    chatSearchMatches = []; chatSearchActiveIndex = -1;
...
function onChatSearchInput(e) {
    ...
    if (!logDiv) return;
    document.querySelectorAll('.message-row.search-hit, .message-row.search-hit-active').forEach(row => row.classList.remove('search-hit', 'search-hit-active'));
    if (!query) { logDiv.classList.remove('chat-searching'); chatSearchMatches = []; chatSearchActiveIndex = -1; updateChatSearchCounter(); return; }
    ...
    logDiv.querySelectorAll('.message-row.me, .message-row.other').forEach(row => {
        ...
        if (text.toLowerCase().includes(query.toLowerCase())) { row.classList.add('search-hit'); chatSearchMatches.push(row); }
    });
...
function updateChatSearchCounter() {
    ...
    if (!counter) return;
    counter.textContent = chatSearchMatches.length === 0 ? '0/0' : `${chatSearchActiveIndex + 1}/${chatSearchMatches.length}`;
}
...
function logMessage(id, text, type, timeStr = "", status = "sent", username = null, replyToId = null) {
    const logDiv = getLogDiv();
    if (!logDiv) return;
    ...
    if (type === "me" || type === "other") {
        ...
        messagesMap.set(id, { id, text, from: type === "me" ? userId : activeTargetId, fromUsername: username || (type === "me" ? localStorage.getItem("username") : activeTargetName) });
...
async function openEncryptionModal() {
    ...
    peerNameLabel.textContent = currentLang === 'ru' ? `Ключ ${activeTargetName}` : `${activeTargetName}'s Key`;
...
function closeEncryptionModal() { document.getElementById('encryptionOverlay').classList.remove('open'); }
...
function xorHexStrings(hex1, hex2) {
    ...
    let result = '';
    for (let i = 0; i < len; i++) result += (parseInt(hex1[i], 16) ^ parseInt(hex2[i], 16)).toString(16).toUpperCase();
    return result;
}
...
function applyTheme(themeIndex) {
    ...
    document.querySelectorAll('.theme-card').forEach((card, idx) => card.classList.toggle('active', idx === themeIndex));
}
...
function startChatWith(targetId, targetName) {
    if (!userCache.has(targetId)) userCache.set(targetId, { username: targetName, lastMessage: null, lastMessageText: null, lastTime: null, lastFromMe: false, unreadCount: 0 });
    renderChatsList();
...
function renderChatsList() {
    ...
    const activeChats = [];
    userCache.forEach((data, uid) => { if (data.lastMessage !== null) activeChats.push({ uid, ...data }); });
    ...
    activeChats.forEach(chat => {
        ...
        if (chat.uid === activeTargetId) btn.classList.add('active');
...
async function loadAllUsers() {
    try {
        ...
        users.forEach(u => {
            ...
            if (!userCache.has(uIdStr)) userCache.set(uIdStr, { username: u.username, lastMessage: null, lastMessageText: null, lastTime: null, lastFromMe: false, unreadCount: 0 });
            else userCache.get(uIdStr).username = u.username;
        });
    } catch (err) { console.log("Failed to load users:", err); }
}

async function loadUsersList() { await loadAllUsers(); await loadLastMessages(); renderChatsList(); }
...
async function loadLastMessages() {
    try {
        const res = await fetch("http://localhost:8080/last-messages", { headers: { "Authorization": "Bearer " + token } });
        if (!res.ok) return;
        ...
        for (const msg of messages) {
            ...
            userCache.set(partnerId, { ...partner, lastMessage: msg, lastMessageText: decrypted, lastTime: msg.created_at, lastFromMe: String(msg.from) === userId });
        }
        renderChatsList();
...
function markAsRead(msgId, fromId) {
    ...
    if (!publicKeys[fromId]) {
        if (!pendingReadQueue.has(fromId)) pendingReadQueue.set(fromId, []);
        pendingReadQueue.get(fromId).push({ id: msgId, from: fromId });
        return;
    }
    ws.send(JSON.stringify({ type: "status_update", id: msgId, to: fromId, status: "read" }));
}
...
function flushPendingReads(fromId) {
    ...
    queue.forEach(({ id, from }) => {
        if (ws && ws.readyState === WebSocket.OPEN) ws.send(JSON.stringify({ type: "status_update", id, to: from, status: "read" }));
    });
}
...
function onTypingInput() {
    if (!activeTargetId || !ws || ws.readyState !== WebSocket.OPEN) return;
    if (!isTyping) { isTyping = true; ws.send(JSON.stringify({ type: "typing", to: activeTargetId })); }
    if (typingTimer) clearTimeout(typingTimer);
    typingTimer = setTimeout(() => {
        isTyping = false;
        if (ws && ws.readyState === WebSocket.OPEN) ws.send(JSON.stringify({ type: "stop_typing", to: activeTargetId }));
    }, TYPING_TIMEOUT);
}
...
function showPeerTyping() {
    ...
    peerTypingTimer = setTimeout(hidePeerTyping, TYPING_TIMEOUT + 500);
}
...
function renderReplyQuote(replyToId) {
    const msg = messagesMap.get(replyToId);
    if (!msg) return null;
...
// ===== СВАЙП — ПЛАВНОЕ ДВИЖЕНИЕ С КОМПЕНСАЦИЕЙ ИКОНКИ =====
function attachSwipeToMessage(row) {
    ...
    // Применяем transform к bubble и компенсируем для иконки
    ...
    function endDrag(targetX, onDone) {
        ...
        // Один RAF чтобы transition активировался
        requestAnimationFrame(() => {
            setTransform(targetX);
        });
        
        // onDone через setTimeout на время transition (350ms)
        if (onDone) {
    ...
    function onMove(clientX, clientY, preventDefault) {
        ...
        if (!isDecided && (Math.abs(diffX) > 5 || Math.abs(diffY) > 5)) {
            isDecided = true;
            isHorizontal = Math.abs(diffX) > Math.abs(diffY) * MIN_HORIZONTAL_RATIO;
            if (isHorizontal) {
        ...
        if (diffX < 0) {
            ...
            if (absDiff > SWIPE_THRESHOLD * 0.75) row.classList.add('swipe-ready');
    ...
    function onEnd() {
        ...
        if (finalX < -SWIPE_THRESHOLD && wasSwipeReady) {
            // Срабатывание reply
            endDrag(-50, () => {
...
function selectUser(targetId, targetName) {
    ...
    renderChatsList();
    updateChatAreaVisibility();
    if (ws && ws.readyState === WebSocket.OPEN) ws.send(JSON.stringify({ type: "get_pubkey", to: activeTargetId }));
    ...
    try {
        ...
        for (let m of messages) {
            ...
            if (text) {
                ...
                const fromUsername = isMe ? localStorage.getItem("username") : (userCache.get(fromIdStr)?.username || "User");
                ...
                const fromIdStr = String(m.from);
                if (fromIdStr !== userId) markAsRead(msgIdStr, fromIdStr);
            }
        }
        if (publicKeys[target]) await showKeyExchangeIfNeeded(target);
        if (messages.length > 0) {
            const lastMsg = messages[messages.length - 1];
            const decrypted = await decryptMessage(lastMsg, userId);
            const isMe = String(lastMsg.from) === userId;
            const currentData = userCache.get(target) || { username: activeTargetName, lastMessage: null, lastMessageText: null, lastTime: null, lastFromMe: false, unreadCount: 0 };
            userCache.set(target, { ...currentData, lastMessage: lastMsg, lastMessageText: decrypted, lastTime: lastMsg.created_at, lastFromMe: isMe });
            renderChatsList();
        }
    } catch { logMessage(null, t('error_history'), "error"); }
}

function initWebSocket() {
    if (ws && (ws.readyState === WebSocket.CONNECTING || ws.readyState === WebSocket.OPEN)) return;
    if (wsReconnectAttempts === 0) updateConnectionStatus('connecting');
    ws = new WebSocket(`ws://localhost:8080/ws?token=${token}`);
    ws.onopen = () => { wsReconnectAttempts = 0; updateConnectionStatus('connected'); ws.send(JSON.stringify({ type: "set_pubkey", pubKey: publicKeyBase64 })); };
    ws.onclose = () => {
        if (!token) return;
        wsReconnectAttempts++;
        if (wsReconnectAttempts <= WS_MAX_RECONNECT) {
            const delay = Math.min(WS_RECONNECT_BASE_DELAY * Math.pow(2, wsReconnectAttempts - 1), WS_RECONNECT_MAX_DELAY);
    ...
    ws.onerror = () => { if (wsReconnectAttempts === 0) updateConnectionStatus('reconnecting'); };
    ws.onmessage = async (e) => {
        ...
        if (d.type === "typing") { if (String(d.from) === activeTargetId) showPeerTyping(); return; }
        if (d.type === "stop_typing") { if (String(d.from) === activeTargetId) hidePeerTyping(); return; }
        if (d.type === "pubkey") {
            publicKeys[d.from] = await crypto.subtle.importKey("spki", fromB64(d.pubKey), { name: "RSA-OAEP", hash: "SHA-256" }, true, ["encrypt"]);
            flushPendingReads(d.from);
            if (String(d.from) === activeTargetId) await showKeyExchangeIfNeeded(d.from);
        ...
        if (d.type === "message") {
            ...
            if (!isMe && fromIdStr === activeTargetId) hidePeerTyping();
            if (!isMe) {
                ...
                if (fromIdStr !== activeTargetId) partnerData.unreadCount = (partnerData.unreadCount || 0) + 1;
            ...
            if (fromIdStr === activeTargetId || (isMe && String(d.to) === activeTargetId)) {
                ...
                if (!isMe && ws && ws.readyState === WebSocket.OPEN) ws.send(JSON.stringify({ type: "status_update", id: msgIdStr, to: fromIdStr, status: "delivered" }));
            }
        }
    };
}
...
async function send() {
    ...
    logMessage(messageId, text, "me", formatTime(), "sent", localStorage.getItem("username"), replyingTo ? replyingTo.id : null);
    cancelReply();
    messageInput.value = "";
    if (typingTimer) clearTimeout(typingTimer);
    isTyping = false;
    if (ws && ws.readyState === WebSocket.OPEN) ws.send(JSON.stringify({ type: "stop_typing", to: target }));
}
...
function logout() {
    ...
    userId = null; activeTargetId = null; activeTargetName = null; currentTab = 'chats';
    for (let key in publicKeys) delete publicKeys[key];
    shownMessages.clear(); userCache.clear(); shownKeyExchanges.clear(); pendingReadQueue.clear();
    if (pendingMessages) Object.keys(pendingMessages).forEach(k => delete pendingMessages[k]);
    chatSearchMatches = []; chatSearchActiveIndex = -1; allUsersList = [];
    cancelReply(); hidePeerTyping(); messagesMap.clear();
    if (typingTimer) clearTimeout(typingTimer);
    if (peerTypingTimer) clearTimeout(peerTypingTimer);
    isTyping = false;
